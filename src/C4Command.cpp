...
void C4Command::MoveTo()
{
	...
	if ((cObj->OCF & OCF_CrewMember) || cObj->Def->Pathfinder)
		if (!PathChecked)
			...
			if (Distance(cx, cy, Tx._getInt(), Ty) < MaxPathRange)
				// Not too close
				if (!(Inside<C4Integer>(cx - Tx._getInt(), -PathRange, +PathRange) && Inside<C4Integer>(cy - Ty, -PathRange, +PathRange)))
				{
	...
	if (Inside<C4Integer>(cx - Tx._getInt(), -iRangeFactorSide * iTargetRange, +iRangeFactorSide * iTargetRange)
...
void C4Command::PushTo()
{
	...
	else
	{
		...
		if (Inside<C4Integer>(Target->x - Tx._getInt(), -PushToRange, +PushToRange))
...
void C4Command::Acquire()
{
	...
	while (pMaterial = Game.FindObject(Data, cObj->x, cObj->y, -1, -1, OCF_Available, nullptr, nullptr, nullptr, nullptr, ANY_OWNER, pMaterial))
		...
		if (!Target2 || pMaterial->Contained != Target2)
			...
			if (Inside<C4Integer>(cObj->x - pMaterial->x, -Tx._getInt(), +Tx._getInt()))
